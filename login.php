<?php
// Token-based authentication - no session checks needed here
    ...
    <style>
        ...
        * {
        ...
        @media (max-width: 480px) {
            ...
            .theme-toggle-btn, .loading-theme-toggle {
                width: 35px;
                height: 35px;
                font-size: 0.9rem;
            }
...
<div class="login-container">
    ...
    <div class="login-form-container">
        ...
        <div class="register-link">
            Don't have an account? <a href="#" onclick="showContactModal()">Create Account</a>
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="background: var(--login-card-bg); border: 1px solid var(--login-border);">
            <div class="modal-header">
                <h5 class="modal-title" style="color: var(--login-text-primary);">
                    <i class="fas fa-info-circle me-2"></i>Account Registration
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body text-center">
                <i class="fas fa-user-plus fa-3x text-muted mb-3"></i>
                <h6 style="color: var(--login-text-primary);">Registration Not Available</h6>
                <p style="color: var(--login-text-muted);">
                    Please contact the BAC Office or MIS Pyo for account creation assistance.
                </p>
                <div class="mt-3">
                    <i class="fas fa-envelope me-2" style="color: var(--login-text-muted);"></i>
                    <small style="color: var(--login-text-muted);">Contact your administrator</small>
                </div>
            </div>
...
// Login Form Handler
// Enhanced debugging for login page
document.addEventListener('DOMContentLoaded', function() {
  console.log('=== LOGIN PAGE DEBUG START ===');
  ...
  try {
    ...
    console.log('localStorage available: NO -', e.message);
  }
...
async function checkExistingLogin() {
    ...
    console.log('=== CHECK EXISTING LOGIN END ===');
    return false; // No redirect happened
}

  console.log('Existing tokens check:');
  console.log('- Access token exists:', !!accessToken);
  console.log('- Refresh token exists:', !!refreshToken);
  console.log('- User data exists:', !!userData);

  console.log('=== LOGIN PAGE DEBUG END ===');
});
document.addEventListener('DOMContentLoaded', function() {
        ...
        if (loadingScreen) {
          ...
          setTimeout(() => {
            ...
            if (loginContainer) {
              ...
              // Check if user is already logged in after loading animation
              setTimeout(async () => {
                const redirected = await checkExistingLogin();
                if (!redirected) {
                  console.log('No valid login found, staying on login page');
                }
              }, 500);
            }
          }, 800);
...
async function handleLogin(event) {
   ...
   try {
      ...
      if (data.status === 'success') {
         ...
         const secureFlag = isLocalhost ? '' : '; secure';
         document.cookie = 'auth_token=' + encodeURIComponent(data.tokens.access_token) +
                          '; path=/' + secureFlag + '; samesite=lax; max-age=86400'; // 24 hours expiry
