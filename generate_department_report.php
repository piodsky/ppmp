...
if ($exportType === 'xlsx') {
    ...
    header('Content-Disposition: attachment;filename="Department_Consolidated_Report_' . date('Y-m-d_H-i-s') . '.xlsx"');
    header('Cache-Control: max-age=0');
} elseif ($exportType === 'csv') {
    header('Content-Type: text/csv');
    header('Content-Disposition: attachment;filename="Department_Consolidated_Report_' . date('Y-m-d_H-i-s') . '.csv"');
    header('Cache-Control: max-age=0');
} else {
    // Default to PDF
...
class ConsolidatedPDF extends FPDF {
    ...
    function TableHeader($departments) {
        ...
        $fixedWidth = 22 + 60 + 15 + 25 + 30 + 30; // Fixed columns width (increased Item Code)
        $availableWidth = $usableWidth - $fixedWidth;
        $deptWidth = max(15, $availableWidth / count($departments)); // Minimum 15mm per dept (half width)
        ...
        // Fixed columns
        $this->Cell(22, $headerHeight, 'Item Code', 1, 0, 'C', true);
        $this->Cell(60, $headerHeight, 'Item & Specifications', 1, 0, 'C', true);
        $this->Cell(15, $headerHeight, 'Unit', 1, 0, 'C', true);
        ...
        $this->Cell(25, $headerHeight, 'Total Qty', 1, 0, 'C', true);
        $this->Cell(30, $headerHeight, 'Unit Cost', 1, 0, 'C', true);
        $this->Cell(30, $headerHeight, 'Total Cost', 1, 1, 'C', true);
    }
    ...
    function TableRow($data, $departments) {
        ...
        $availableWidth = $usableWidth - $fixedWidth;
        $deptWidth = max(15, $availableWidth / count($departments)); // Minimum 15mm per dept (half width)
        $widths = array_merge([22, 60, 15], array_fill(0, count($departments), $deptWidth), [25, 30, 30]);
...
if ($exportType === 'xlsx') {
    // Generate XLSX file
    generateDepartmentXLSX($groupedItems, $departments, $currentYear, $approvedCount);
} elseif ($exportType === 'csv') {
    // Generate CSV file
    generateDepartmentCSV($groupedItems, $departments, $currentYear, $approvedCount);
} else {
    // Default PDF output
    $filename = 'Department_Consolidated_Report_' . date('Y-m-d_H-i-s') . '.pdf';
...
// Function to generate XLSX export for department report
function generateDepartmentXLSX($groupedItems, $departments, $currentYear, $approvedCount) {
    ...
    $spreadsheet->getProperties()
        ...
        ->setTitle('Department Consolidated Report')
        ->setSubject('Consolidated Procurement Items by Department')
        ->setDescription('Generated department consolidated report for approved PPMP documents');
...
// Function to generate CSV export for department report
function generateDepartmentCSV($groupedItems, $departments, $currentYear, $approvedCount) {
    // Create CSV header
